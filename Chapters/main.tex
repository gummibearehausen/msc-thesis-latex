...
This discretization method consists of simply querying both the maximum and minimum values of the root's numerical
attribute, then for splitting this interval into $k$ buckets of same width $w$:
\begin{equation}
 w=cfrac{max-min}{k}
\end{equation}

\subsection{Equal Frequencies}
In this discretization method, frequencies are summed up then the result is divided by $k$. Bucket widths may be
different and should adjusted in order to obtain buckets with equal frequencies. Nevertheless, data distribution might
make it impossible to split it equally, so boundaries are set in order to have an overall distribution as close as
possible to uniformity.
