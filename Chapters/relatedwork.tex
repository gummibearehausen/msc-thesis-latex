...
a variable or a function symbol followed by a bracketed n-tuple of terms, a \emph{constant} is a function symbol of
arity 0. So for example, if $f$, $g$, and $h$ are function symbols and $X$ a variable, then $g(X)$ is term, $f(g(X),h)$
...
interval, as requires in this thesis.

\section{Inductive Logic Programming}
\label{sec:rw-ilp}

Inductive Logic Programming (ILP) is a rule learning framework that combines machine learning with the
...
data, however, we do not discuss them since it is out of the scope of this thesis.
...
The confidence of a rule $X \Rightarrow Y$, which can also be interpreted as the probability of the the head given the
...
When searching for frequent itemsets, this characteristic plays a fundamental role in pruning the search space, which
grows exponentially with the number of items $n$, since the set of possible itemsets is the power set over
$\mathcal{I}$, which has size $2^n-1$. As association rules are usually required to satisfy a specified minimum support,
if we know that a given itemset does not satisfy it, we can automatically prune all its supersets.
...
itemsets. By knowing all the closed itemsets of a database, it is possible deduce not only all the frequent itemsets,
...
threshold is set to $2/5$, so the frequent itemsets are the ones with frequency greater than or equal to 2. In this
...
ascending order of size. The process takes $k$ iterations, where $k$ is the size of the largest frequent itemset. In
...
of size $i+1$. Only after finding the set of candidate itemsets $C_1$, the database is scanned in order to determine the
...
and served as base for our proposed correlation lattice. Therefore, both structures are notably similar, and they share
...
of buckets to be beforehand defined.

Various different supervised and unsupervised methods for the discretization of continuous features are discussed in
\citet{Dougherty95supervisedand}. To build the lattice, we do not consider labels for the examples, so we use
unsupervised methods only. The discretization of numerical attributes plays a key role in this thesis, as it allows us
...
examples as labels. However, as it is out of the scope of this thesis, we do not discuss these in detail.
...
It's the simplest and most straightforward discretization method, howeverm, its main problem is that it is highly
sensitive to outliers, which may drastically skew the resulting distribution.

\subsection{Equal Frequencies}

In this another unsupervised discretization method, the domain is divided in buckets with equal frequency. That is,
given a total frequency $m$, the domain is discretized in $k$ buckets such that each has frequency $m/k$. 

Multiple objects with same numerical attribute value might make it impossible to find bucket boundaries that equally
...
a long this numerical attribute.
...
is less (negative gain). This method does not compromise the optimality of the algorithm, since it collapses values with
similar labels together, generating $b$ buckets containing equally labeled objects, each with its own support,
confidence, and gain values. Figure~\ref{fig:brinbucket} shows an example where a domain of size $n=6$ is discretized
into $b=3$ buckets.
...
$10$, $-15$, $20$, $-15$, $20$, and $-15$. $PSet$ is the optimized gain set containing $i-1$ intervals, and $NSet$
contains the remaining intervals not stored in $PSet$. These sets are represented before and after the iterations $i=1$
and $i=2$. As shown in the figure, any optimized interval contained in $PSet$ has positive gain, and its neighboring
intervals have negative gain.
...
predicting the interestingness of searching for refinements for a numerical attribute variable before we actually query
the support and confidence distributions, allowing us to prune uninteresting rules avoiding expensive queries.
...
power to measure importance of attributes and relationships between attributes. An attribute is considered important in
data mining if regularities are encountered in smaller populations obtained based on the values of such attribute, but
not in the larger population. Such regularities are identifiable by lower entropy values, with interesting
attributes causing entropy reduction. More specifically, this entropy reduction is the difference between the entropy
of the decision attribute and the conditional entropy of the decision attribute given a particular attribute, whose
interestingness we want to investigate.
...
is done by simply measuring the divergence between the observed joined distribution $P(X,Y)$ and the independent
...
This sort of linkage allows applications to exploit the extra information these datasets can provide by integrating
facts from various different sources. Figure~\ref{fig:lod} shows the latest version of the whole Linked Open Data cloud
