...
Let's say we have $X$ as decision attribute and it divides the set of objects $U$ into a group of disjoint subsets
defined by the value $x \in V_X$ of $X$, where $V_X$ is the non-empty set of values for $X$. Also, let's define $I_X :
U \rightarrow V_X$ is an information function that gives the value of attribute $X$ for an object $t \in U$. The set of
objects $m(X=x) \subseteq U$, whose value for $X$ is $x$ is defined as follows:

\begin{equation}
 m(X=x)=m(x)=\{t \in U | I_X(t)=x\}
\end{equation}

With that, the probability distribution of $X$ is defined by:

\begin{equation}
 P(X=x)=P(x)=\cfrac{|m(x)|}{|U|},\quad x \in V_X
\end{equation}
