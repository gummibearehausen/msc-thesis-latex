%\externaldocument{algorithm-framework}
%\externaldocument{relatedwork}
%\externaldocument{introduction}
%\externaldocument{experiments}
...
be used for joining with all the categorical properties.
...
given example, with the relations \emph{hasEducation} and \emph{quarterOfBirth}, we would then create the nodes:
 ...
 \begin{figure}[!h]
  ...
  \begin{tikzpicture}[scale=1.75,auto=center,every node/.style={,font=\tiny}]
    ...
    \node[text width=1cm,align=center] (a1) 	at (1,3) {$hasIncome(X,Y),$\\$quarterOfBirth(X,q1)$};
    \node[text width=1cm,align=center] (an)	at (3,3) {$hasIncome(X,Y),$\\$quarterOfBirth(X,q4)$};
...
the clause is interesting by calling the function \FuncSty{isClauseInteresting}, which is shown in Algorithm~\ref{alg6}.
...
Minimum interestingness value allowed \\}
  ...
  \If{$arg_{new} = 1$ \textbf{and} $r_{new}$ is numerical \textbf{and} $\exists$ $lattice(r_{new})$} {
    ...
    \If{\FuncSty{isClauseInteresting(}$clause$,$root$,$interestingnessThreshold$\FuncSty{)}} {
...
\begin{algorithm}[!h]
 \caption{\FuncSty{isClauseInteresting}}
 \SetKwFunction{checkNumericalRanges}
...
Once we find out that that the clause is interesting, we can perform the numerical interval search refinement
...
\FuncSty{searchInterestingIntervals}. If any relevant interval is found, we add the required the arithmetic literals to
