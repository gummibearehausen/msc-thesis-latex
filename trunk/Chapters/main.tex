...
\emph{isParentOf(x,y)} is not categorical but relational, and both its domain and range are from the type Person. If not
all the Person instances of the knowledge base have supporting facts for the relation \emph{isParentOf}, then we can
...
\begin{tabular}{*{3}{l}}
type(John,Person) & hasIncome(John,20000) & isParentOf(John,Mary) \\
type(Mike,Person) & hasIncome(Mike,30000) & isParentOf(Mike,Paul) \\
type(Anne,Person) & hasIncome(Anne,25000) & isParentOf(Anne,Paul) \\
type(Mary,Person) & hasIncome(Mary, 5000) & isParentOf(Anne,Mary) \\
type(Lisa,Person) & hasIncome(Lisa,10000) & \\
type(Paul,Person) & & \\
\end{tabular}

Then we could split the root node set $X_{root}=\{x|hasIncome(x,z)\}$ in two categories $X_{root}^1,X_{root}^2 \in
X_{root}$:
...
$X_{root}=\{John,Mike,Anne,Mary,Lisa\} \\$
