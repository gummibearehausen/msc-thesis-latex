...
In figure \ref{interestingnessExample}, we illustrate that with an ideal example where positive examples and body
support have completely different distributions and thus produce a very interesting confidence distribution. Such
distribution is obtained thanks to the divergent rule's positives and body support distributions. This is shown in
figure \ref{interestingnessDistribution}, where we obtained these distributions by normalizing the frequency histograms
...
A candidate categorical relation $r(X,Z)$, where $X \in \mathcal{X}$ is the join variable and $Z$ is within
...
Typically, $\sum_{i=1}^{k}|\mathcal{X}_{z_i}| \ll k$ and $\mathcal{X}_{z_i} \subseteq
...
as linking relation. This allows to broaden the set of categorical properties to be analyzed in the \graphname to those
...
approach, one can also include non-categorical properties into the \graphname and have insight about how the presence or
absence of such property affects the distribution of root's numerical attribute.
...
As we will see in the next sections, in the \graphname all the relations are joined by the variable of root's join
argument $X$. So we will denote the presence of a property $a(X,Z)$ category as simply $a$, where the variable $Z$ is
free and not set to any constant. If we have a categorical property $a(X,z_i)$ with $k$ different categories $z_i \in
...
Every node in the lattice contains a frequency histogram of its population, and all histograms are built on the same
buckets defined in the root node for the sake of comparison. 
...
\begin{figure}[!h]
  ...
  [scale=1,auto=center,every node/.style={minimum size=1cm}]
    ...
    \node (n1) [circle,fill=black!20] at (3,8)  {$px$};
    \node (n2) [circle,fill=black!20] at (5,8)  {$py$};
    \node (n12)[circle,fill=black!20] at (4,6)  {$pxy$};
